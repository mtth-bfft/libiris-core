...
pub trait CrossPlatformSandboxedProcess {
    ...
    fn new(policy: &Policy, exe: &CStr, argv: &[&CStr], envp: &[&CStr]) -> Result<Self, String> where Self: std::marker::Sized;

    // Returns the ID of the worker process
    fn get_pid(&self) -> u64;

    // Blocks until the process exits, and returns it exit code
