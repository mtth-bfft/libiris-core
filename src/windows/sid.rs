...
impl Sid {
    pub(crate) fn from_appcontainer_name(name: &str) -> Result<Self, String> {
        ...
        let res = unsafe { DeriveAppContainerSidFromAppContainerName(name_buf.as_ptr(), &mut psid as *mut _) };
        if res != 0 || psid.is_null() {
            return Err(format!("DeriveAppContainerSidFromAppContainerName() failed with error {}", unsafe { GetLastError() }));
        }
        Ok(Self {
            psid,
        })
...
impl fmt::Display for Sid {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if res == 0 {
            return write!(f, "<Unable to format SID: error {}>", unsafe { GetLastError() });
        }
        let sid = unsafe { CStr::from_ptr(pstr) };
        let res = write!(f, "{}", &sid.to_string_lossy());
        unsafe { LocalFree(pstr as *mut _); }
        res
    }
}
